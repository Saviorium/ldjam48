local particleTypes = {
    dirtChunk = {
        ...
        friction = 0.99,
        spawnAt = Vector(0, 0),
        angle = 180, -- deg, aligned with drill, clockwise
        speed = 2,
...
function Particle:spawn(position, angleGlobal) -- angle in deg, from up, clockwise
    ...
    self.color[1] = self.type.color[1] + self.type.color[1]*random(self.type.random.color) -- optimisations, reusing...
    self.color[2] = self.type.color[2] + self.type.color[2]*random(self.type.random.color)
    self.color[3] = self.type.color[3] + self.type.color[3]*random(self.type.random.color)
    ...
    local angleInRad = 2 * math.pi * angleGlobal + random(self.type.random.angle)
...
function DrillParticleSystem:update(dt)
    ...
    for type, particlesOfType in pairs(self.particles) do
        ...
        local angle = 0 -- rotate with the drill
